...
class Admin {
	...
	public function hooks() {
		...
		// Image-link fixer.
		add_action( 'wp_ajax_rd3_pic_scan_image_links', array( $this, 'ajax_scan_image_links' ) );
		add_action( 'wp_ajax_rd3_pic_fix_image_links', array( $this, 'ajax_fix_image_links' ) );
	}
		...
		wp_localize_script(
			...
			array(
				...
				'i18n'    => array(
					...
					'imageLinksScanning' => __( 'Scanning Posts and Pages for image links…', 'rd3-post-image-cleanup' ),
					'imageLinksScanDone' => __( 'Image-link scan complete.', 'rd3-post-image-cleanup' ),
					'imageLinksScanError' => __( 'Image-link scan failed.', 'rd3-post-image-cleanup' ),
					'imageLinksConfirm' => __( "This will update matching image links in Posts and Pages.\n\nThe link will continue to open the full original image, while the displayed image will use a WordPress-generated smaller size with responsive srcset support.\n\nNo files will be deleted.\n\nContinue?", 'rd3-post-image-cleanup' ),
					'imageLinksFixing' => __( 'Fixing image links…', 'rd3-post-image-cleanup' ),
					'imageLinksFixed' => __( 'Image-link fix complete.', 'rd3-post-image-cleanup' ),
					'imageLinksFixError' => __( 'Image-link fix failed.', 'rd3-post-image-cleanup' ),
				),
			)
		);
	}
	...
	public function render_page() {
		...
		<div class="wrap rd3-pic-wrap">
			...
			<p class="description">
				<?php echo esc_html__( 'Visual duplicate review, optional cleanup, large-image handling, and image-link repair. Files are never deleted.', 'rd3-post-image-cleanup' ); ?>
			</p>
			...
			<div class="rd3-pic-card">
				...
				<p>
					<?php echo esc_html__( 'This scan is completely read-only. It will not edit posts, move files, or delete anything.', 'rd3-post-image-cleanup' ); ?>
				</p>
			...
			<div class="rd3-pic-card">
				...
				<p>
					<?php echo esc_html__( 'Finds full-size images embedded in posts and shows them so you can review. Then optionally rewrites the display src to ~768px while the click link stays on the full original.', 'rd3-post-image-cleanup' ); ?>
				</p>
			...
			<div class="rd3-pic-card">
				...
				<h2><?php echo esc_html__( 'Image links in Posts & Pages', 'rd3-post-image-cleanup' ); ?></h2>

				<p>
					<?php echo esc_html__( 'Looks specifically for images wrapped in an HTML link where the image itself is using the large/full-size image. The link is kept pointing to the full original, but the displayed image is changed to a WordPress-generated smaller size with responsive srcset support.', 'rd3-post-image-cleanup' ); ?>
				</p>

				<p>
					<strong><?php echo esc_html__( 'This tool checks both Posts and Pages.', 'rd3-post-image-cleanup' ); ?></strong>
				</p>

				<p>
					<button type="button" id="rd3-pic-scan-image-links-btn" class="button button-primary">
						<?php echo esc_html__( 'Scan Image Links', 'rd3-post-image-cleanup' ); ?>
					</button>

					<button type="button" id="rd3-pic-fix-image-links-btn" class="button" disabled>
						<?php echo esc_html__( 'Fix Image Links', 'rd3-post-image-cleanup' ); ?>
					</button>
				</p>

				<div id="rd3-pic-image-links-status" class="rd3-pic-status" aria-live="polite"></div>

				<div id="rd3-pic-image-links-results" style="display:none;"></div>
			...
			<div class="rd3-pic-card">
				...
				<p>
					<label for="rd3-pic-merge-keep">
						<strong><?php echo esc_html__( 'KEEP (master) filename', 'rd3-post-image-cleanup' ); ?></strong>
					</label><br />
				...
				<p>
					<label for="rd3-pic-merge-remove">
						<strong><?php echo esc_html__( 'REMOVE (duplicate) filename', 'rd3-post-image-cleanup' ); ?></strong>
					</label><br />
			...
			<div class="rd3-pic-card">
				...
				<p>
					<?php echo esc_html__( 'Enter the exact image filename. Scan finds which posts use it. Then replace large full-size embeds with a ~768px display image, while the click link still opens the full original.', 'rd3-post-image-cleanup' ); ?>
				</p>
			...
			<div class="rd3-pic-card">
				...
				<p>
					<strong><?php echo esc_html__( 'No files will be deleted.', 'rd3-post-image-cleanup' ); ?></strong>
				</p>
	...
	public function render_results( array $results ) {
		...
		<div class="rd3-pic-summary">
			...
			<?php if ( ! empty( $results['scanned_at'] ) ) : ?>
				<p class="description">
					<?php echo esc_html( sprintf( __( 'Scanned at: %s', 'rd3-post-image-cleanup' ), $results['scanned_at'] ) ); ?>
				</p>
			<?php endif; ?>
		</div>
		...
		<?php if ( empty( $groups ) ) : ?>
			...
			<p>
				<em><?php echo esc_html__( 'No exact duplicate original images found among posts.', 'rd3-post-image-cleanup' ); ?></em>
			</p>
		...
		foreach ( $groups as $hash => $group ) {
			...
			<div class="rd3-pic-group">
				...
				<h3>
					<?php echo esc_html( sprintf( __( 'Duplicate Group #%d', 'rd3-post-image-cleanup' ), $group_num ) ); ?>
				</h3>

				<p>
					<strong><?php echo esc_html__( 'SHA-256:', 'rd3-post-image-cleanup' ); ?></strong>
					<code><?php echo esc_html( $hash ); ?></code>
				</p>

				<p>
					<strong><?php echo esc_html__( 'Copies:', 'rd3-post-image-cleanup' ); ?></strong>
					<?php echo esc_html( (string) ( 1 + count( $duplicates ) ) ); ?>
				</p>

				<div class="rd3-pic-master">

					<h4>
						<?php echo esc_html__( 'MASTER (proposed) — look at the image', 'rd3-post-image-cleanup' ); ?>
					</h4>
					...
					<table class="widefat">
						<tbody>
							<tr>
								...
								<td><?php echo esc_html( (string) ( $master['attachment_id'] ?? '' ) ); ?></td>
							</tr>
							<tr>
								<th><?php echo esc_html__( 'Filename', 'rd3-post-image-cleanup' ); ?></th>
								<td><?php echo esc_html( $master['filename'] ?? '' ); ?></td>
							</tr>
							<tr>
								<th><?php echo esc_html__( 'File path', 'rd3-post-image-cleanup' ); ?></th>
								<td><code><?php echo esc_html( $master['path'] ?? '' ); ?></code></td>
							</tr>
							<tr>
								<th><?php echo esc_html__( 'File size', 'rd3-post-image-cleanup' ); ?></th>
								<td><?php echo esc_html( size_format( $master['filesize'] ?? 0 ) ); ?></td>
							</tr>
							<tr>
								<th><?php echo esc_html__( 'Dimensions', 'rd3-post-image-cleanup' ); ?></th>
								<td><?php echo esc_html( ( $master['width'] ?? '?' ) . ' × ' . ( $master['height'] ?? '?' ) ); ?></td>
							</tr>
							<tr>
								<th><?php echo esc_html__( 'Upload date', 'rd3-post-image-cleanup' ); ?></th>
								<td><?php echo esc_html( $master['upload_date'] ?? '' ); ?></td>
							</tr>
							<tr>
								<th><?php echo esc_html__( 'Posts using it', 'rd3-post-image-cleanup' ); ?></th>
								<td><?php echo esc_html( (string) ( $master['post_count'] ?? 0 ) ); ?></td>
							</tr>
						</tbody>
					</table>
							...
							<tbody>
								...
								<?php foreach ( $posts as $p ) : ?>
									...
									<tr>
										...
										<td>
											<code class="rd3-pic-url"><?php echo esc_html( $p['url'] ?? '' ); ?></code>
										</td>
									</tr>
	...
	public function ajax_clear_results() {
		...
		wp_send_json_success(
			array(
				'message' => __( 'Results cleared.', 'rd3-post-image-cleanup' ),
			)
		);
	}
	...
	public function render_large_results( array $report ) {
		...
		<div class="rd3-pic-summary">
			...
			<?php if ( ! empty( $report['scanned_at'] ) ) : ?>
				<p class="description">
					<?php echo esc_html( sprintf( __( 'Scanned at: %s', 'rd3-post-image-cleanup' ), $report['scanned_at'] ) ); ?>
				</p>
			<?php endif; ?>
		...
		<?php if ( empty( $items ) ) : ?>
			...
			<p>
				<em><?php echo esc_html__( 'No large full-size image embeds found in posts.', 'rd3-post-image-cleanup' ); ?></em>
			</p>
		...
		<p class="description">
			<?php echo esc_html__( 'Review each image below. Proposed change: display a ~768px version; clicking still opens the full original.', 'rd3-post-image-cleanup' ); ?>
		</p>
				...
				<div class="rd3-pic-large-card">
					...
					<div class="rd3-pic-large-meta">
						...
						<p class="description">
							...
							<code><?php echo esc_html( basename( $item['proposed_src'] ) ); ?></code><br />

							<strong><?php echo esc_html__( 'href →', 'rd3-post-image-cleanup' ); ?></strong>
							<code><?php echo esc_html( basename( $item['proposed_href'] ) ); ?></code>
						</p>
	...
	public function ajax_scan_named() {
		...
		try {
			...
			wp_send_json_success(
				array(
					'message'  => ! empty( $report['message'] ) ? $report['message'] : __( 'Named-image scan complete.', 'rd3-post-image-cleanup' ),
					'html'     => $html,
					'canRun'   => $can_run,
					'filename' => $filename,
				)
	...
	public function render_named_results( array $report ) {
		...
		<?php if ( empty( $attachments ) ) : ?>
			...
			<p>
				<em><?php echo esc_html__( 'No media attachment matched that filename.', 'rd3-post-image-cleanup' ); ?></em>
			</p>
			...
			<?php foreach ( $attachments as $att ) : ?>
				...
				<div class="rd3-pic-large-card">
					...
					<div class="rd3-pic-large-meta">
						...
						<p>
							<strong><?php echo esc_html( $att['filename'] ); ?></strong>
							(#<?php echo esc_html( (string) $att['attachment_id'] ); ?>)
						</p>

						<p>
							<?php echo esc_html( (int) $att['width'] . ' × ' . (int) $att['height'] . ' · ' . size_format( (int) $att['filesize'] ) ); ?>
						</p>
		...
		<?php if ( empty( $usages ) ) : ?>
			...
			<p>
				<em><?php echo esc_html__( 'This file is in the media library but was not found in any post content or featured image.', 'rd3-post-image-cleanup' ); ?></em>
			</p>
			...
			<tbody>
				...
				<?php foreach ( $usages as $u ) : ?>
					...
					<tr>
						...
						<td>
							<?php echo esc_html( implode( ', ', $u['roles'] ?? array() ) ); ?>
						</td>
	...
	public function render_merge_preview( array $preview ) {
		...
		<?php if ( ! empty( $preview['message'] ) ) : ?>
			<p>
				<strong><?php echo esc_html( $preview['message'] ); ?></strong>
			</p>
		<?php endif; ?>
		...
		<div class="rd3-pic-merge-pair">
			...
			<?php if ( $keep ) : ?>
				...
				<div class="rd3-pic-large-card rd3-pic-keep-card">
					...
					<p class="rd3-pic-badge rd3-pic-badge-keep">
						<?php echo esc_html__( 'KEEP', 'rd3-post-image-cleanup' ); ?>
					</p>
					...
					<div class="rd3-pic-large-meta">
						...
						<p>
							<strong><?php echo esc_html( $keep['filename'] ); ?></strong>
							(#<?php echo esc_html( (string) $keep['attachment_id'] ); ?>)
						</p>

						<p>
							<?php echo esc_html( (int) $keep['width'] . ' × ' . (int) $keep['height'] . ' · ' . size_format( (int) $keep['filesize'] ) ); ?>
						</p>
			...
			<?php if ( $remove ) : ?>
				...
				<div class="rd3-pic-large-card rd3-pic-remove-card">
					...
					<p class="rd3-pic-badge rd3-pic-badge-remove">
						<?php echo esc_html__( 'REMOVE → duplicate-images/', 'rd3-post-image-cleanup' ); ?>
					</p>
					...
					<div class="rd3-pic-large-meta">
						...
						<p>
							<strong><?php echo esc_html( $remove['filename'] ); ?></strong>
							(#<?php echo esc_html( (string) $remove['attachment_id'] ); ?>)
						</p>

						<p>
							<?php echo esc_html( (int) $remove['width'] . ' × ' . (int) $remove['height'] . ' · ' . size_format( (int) $remove['filesize'] ) ); ?>
						</p>
		...
		<?php if ( ! empty( $posts ) ) : ?>
			...
			<h4>
				<?php echo esc_html__( 'Posts that will be updated (currently use REMOVE image)', 'rd3-post-image-cleanup' ); ?>
			</h4>
				...
				<tbody>
					...
					<?php foreach ( $posts as $p ) : ?>
						...
						<tr>
							...
							<td>
								<?php echo esc_html( implode( ', ', $p['roles'] ?? array() ) ); ?>
							</td>
		...
		<?php elseif ( ! empty( $preview['ok'] ) ) : ?>
			...
			<p>
				<em><?php echo esc_html__( 'No posts currently reference the REMOVE image (it may still be moved if you run merge).', 'rd3-post-image-cleanup' ); ?></em>
			</p>
	...
	/**
	 * AJAX: scan image links in Posts and Pages.
	 */
	public function ajax_scan_image_links() {
		check_ajax_referer( 'rd3_pic_admin', 'nonce' );

		if ( ! current_user_can( self::CAP ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'Permission denied.', 'rd3-post-image-cleanup' ),
				),
				403
			);
		}
		...
		try {
			$fixer  = new Image_Link_Fixer();
			$report = $fixer->scan();

			set_transient( 'rd3_pic_image_link_scan', $report, DAY_IN_SECONDS );

			ob_start();
			$this->render_image_link_results( $report );
			$html = ob_get_clean();

			wp_send_json_success(
				array(
					'message' => __( 'Image-link scan complete.', 'rd3-post-image-cleanup' ),
					'html'    => $html,
					'count'   => (int) ( $report['match_count'] ?? 0 ),
					'canRun'  => ! empty( $report['items'] ),
				)
			);
	...
	/**
	 * AJAX: fix image links in Posts and Pages.
	 */
	public function ajax_fix_image_links() {
		check_ajax_referer( 'rd3_pic_admin', 'nonce' );
		...
		try {
			...
			$summary = '<ul class="rd3-pic-cleanup-stats">'
				. '<li>' . esc_html( sprintf( __( 'Posts updated: %d', 'rd3-post-image-cleanup' ), $stats['posts_updated'] ?? 0 ) ) . '</li>'
				. '<li>' . esc_html( sprintf( __( 'Pages updated: %d', 'rd3-post-image-cleanup' ), $stats['pages_updated'] ?? 0 ) ) . '</li>'
				. '<li>' . esc_html( sprintf( __( 'Image links fixed: %d', 'rd3-post-image-cleanup' ), $stats['images_changed'] ?? 0 ) ) . '</li>'
				. '<li>' . esc_html( sprintf( __( 'Skipped: %d', 'rd3-post-image-cleanup' ), $stats['skipped'] ?? 0 ) ) . '</li>'
				. '<li>' . esc_html( sprintf( __( 'Errors: %d', 'rd3-post-image-cleanup' ), $stats['errors'] ?? 0 ) ) . '</li>'
				. '</ul>';
			...
			if ( empty( $result['success'] ) ) {
				wp_send_json_error(
					array(
						'message'     => $result['message'] ?? __( 'Image-link fix failed.', 'rd3-post-image-cleanup' ),
						'summaryHtml' => $summary,
						'logHtml'     => $log_html,
					)
				);
			...
			wp_send_json_success(
				array(
					'message'     => $result['message'] ?? __( 'Image-link fix complete.', 'rd3-post-image-cleanup' ),
					'summaryHtml' => $summary,
					'logHtml'     => $log_html,
				)
			);
	...
	/**
	 * Render image-link scan results.
	 *
	 * @param array $report Scan report.
	 */
	public function render_image_link_results( array $report ) {
		$items = $report['items'] ?? array();
		?>
		<div class="rd3-pic-summary">
			...
			<ul>
				<li>
					<strong><?php echo esc_html__( 'Posts scanned:', 'rd3-post-image-cleanup' ); ?></strong>
					<?php echo esc_html( (string) ( $report['posts_scanned'] ?? 0 ) ); ?>
				</li>

				<li>
					<strong><?php echo esc_html__( 'Pages scanned:', 'rd3-post-image-cleanup' ); ?></strong>
					<?php echo esc_html( (string) ( $report['pages_scanned'] ?? 0 ) ); ?>
				</li>

				<li>
					<strong><?php echo esc_html__( 'Image links found:', 'rd3-post-image-cleanup' ); ?></strong>
					<?php echo esc_html( (string) ( $report['match_count'] ?? 0 ) ); ?>
				</li>
		...
		<?php if ( empty( $items ) ) : ?>
			...
			<p>
				<em>
					<?php echo esc_html__( 'No matching large/full-size image links were found.', 'rd3-post-image-cleanup' ); ?>
				</em>
			</p>
		...
		<p class="description">
			<?php echo esc_html__( 'These are the links that will be changed. The href remains the full-size image; only the displayed image source and responsive attributes are changed.', 'rd3-post-image-cleanup' ); ?>
		</p>
		...
		<div class="rd3-pic-large-grid">
			...
			<?php foreach ( $items as $item ) : ?>
				...
				<div class="rd3-pic-large-card">
					...
					<a href="<?php echo esc_url( $item['href'] ); ?>" target="_blank" rel="noopener">
						<img src="<?php echo esc_url( $item['current_src'] ); ?>" alt="" />
					</a>
					...
					<div class="rd3-pic-large-meta">
						...
						<p>
							<strong><?php echo esc_html__( 'Current:', 'rd3-post-image-cleanup' ); ?></strong>
							<code><?php echo esc_html( basename( $item['current_src'] ) ); ?></code>
						</p>

						<p>
							<strong><?php echo esc_html__( 'Will become:', 'rd3-post-image-cleanup' ); ?></strong>
							<code><?php echo esc_html( basename( $item['proposed_src'] ) ); ?></code>
						</p>

						<p>
							<strong><?php echo esc_html__( 'Full link:', 'rd3-post-image-cleanup' ); ?></strong>
							<code><?php echo esc_html( basename( $item['href'] ) ); ?></code>
						</p>

						<p>
							<a href="<?php echo esc_url( get_edit_post_link( $item['post_id'] ) ); ?>" target="_blank" rel="noopener">
								<?php echo esc_html__( 'Edit content', 'rd3-post-image-cleanup' ); ?>
							</a>
						</p>
